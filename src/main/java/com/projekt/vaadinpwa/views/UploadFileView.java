...
public class UploadFileView extends VerticalLayout {
    ...
    private TreeGrid<FileEntity> folderGrid = new TreeGrid<>(FileEntity.class);
    private Button newFolderButton = new Button("Utwórz folder");
    private TextField newFolderName = new TextField();
    private H3 choosenFolderLabel = new H3();
    ...
    public UploadFileView(FileService fileService, UserService userService) {
        ...
        newFileLayout.add(
                choosenFolderLabel = new H3("Wybrany folder to: " + (selectedPath.equals("") ? "katalog główny" : selectedPath)),
                newFolderName,
                newFolderButton
        );
        newFileLayout.expand(choosenFolderLabel);
    ...
    private void configureUploader() {
        ...
        upload.addSucceededListener(event -> {
            fileService.uploadFile(event.getFileName(), selectedPath, buffer.getInputStream(), event.getContentLength(), loggedUser);
            Notification.show(
    ...
    private void configureGrid() {
        ...
        folderGrid.addComponentColumn(file -> VaadinIcon.FOLDER_OPEN_O.create()
        ...
        folderGrid.asSingleSelect().addValueChangeListener(e -> {
            ...
            choosenFolderLabel.setText("Wybrany folder to: " + (selectedPath.equals("") ? "katalog główny" : selectedPath));
        });
    ...
    private void configureNewFolderButtonAndTextField() {
        ...
        newFolderButton.addClickListener(e -> {
                    fileService.createNewDirectory(newFolderName.getValue(), selectedPath + newFolderName.getValue() + "/", selectedDirectory, loggedUser);
